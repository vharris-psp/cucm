...
The selected user's CUCM primary extension is authoritative when it is a four-digit DN; a four-digit LDAP telephone number is used only when no primary extension is assigned. The local user-DN inventory is an allocation fallback only when neither assigned value is available, and only fallback allocations are recorded in that inventory.
